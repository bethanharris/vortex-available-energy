...
class Vortex:
    ...
    def environment_pressure(self, z):
        return self.far_field_pressure * np.exp((self.g*np.log(1. - self.lapse_rate*z))/(self.Rd*self.surface_temperature*self.lapse_rate))

    def environment_pressure_vertical_gradient(self, z):
        return -(self.far_field_pressure*self.g*(1. - self.lapse_rate*z)**(self.g/(self.Rd*self.surface_temperature*self.lapse_rate) - 1.))/(self.Rd*self.surface_temperature)
    ...
    def pressure_vertical_gradient(self, r, z):
        ...
        vertical_term = -np.exp(-z / self.vertical_scale) * (1. / self.vertical_scale) * (
                    (0.5 * np.pi * self.vertical_scale / self.vertical_top) * np.sin(0.5 * np.pi * z / self.vertical_top) + np.cos(
    ...
    def pressure_radial_gradient(self, r, z):
        radial_term = -self.radial_coefficient*self.radial_scale*np.exp(-self.radial_coefficient*self.radial_scale/r)/(r**2)
        vertical_term = np.exp(-z / self.vertical_scale) * np.cos(0.5 * np.pi * z / self.vertical_top)
        return self.pressure_deficit * radial_term * vertical_term
